...
3. Request only the five scopes named above, plus `instagram_manage_insights`, `instagram_manage_comments`, and `instagram_manage_engagement` for the dashboard's Analytics/Comments panels (see `dashboard/server/comments.ts`, `dashboard/server/ig.ts`) — none of these need a separate app or OAuth flow, they're just additional scopes on this same token. Standard Access with yourself as an app admin/tester is enough — no public App Review needed since this only ever publishes to accounts you own (see `META_AUDIT_SUBMISSION.md` if a review is ever requested). If any scope shows as "Invalid Scopes" during auth, see **Troubleshooting** below — the use-case wizard doesn't always fully attach every permission it implies.
...
> **Note on comment like/unlike:** `POST/DELETE /{ig-user-id}/likes` is documented at `graph.facebook.com` under Meta's classic Instagram Graph API reference, requiring only `instagram_basic` + `instagram_manage_engagement` on this same Page token — **not** a separate Instagram Login / Business Login for Instagram flow. An earlier version of this doc (and the corresponding code) incorrectly assumed it needed a second app; see the correction note in `docs/superpowers/specs/2026-07-02-instagram-login-comment-engagement-design.md` for the full story.
