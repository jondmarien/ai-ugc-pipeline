...
import { multipleCaptionsEnabled, PostData, validatePost } from "./schema";
...
test("rejects slide_captions when flag is off", () => {
  ...
  if (!parsed.success) {
    expect(
      parsed.error.issues.some((i) => i.path.join(".") === "slide_captions"),
    ).toBe(true);
  }
});
  ...
  if (!parsed.success) {
    expect(
      parsed.error.issues.some((i) =>
        String(i.message).includes("slide_captions count"),
      ),
    ).toBe(true);
  }
});
...
test("rejects empty slide_captions entry when flag is on", () => {
  const n = (basePost.slides as unknown[]).length;
  const slide_captions = Array.from({ length: n }, (_, i) =>
    i === 2 ? "   " : `Caption ${i + 1}.`,
  );
  const raw = {
    ...basePost,
  ...
  if (!parsed.success) {
    expect(
      parsed.error.issues.some((i) => i.path.join(".") === "slide_captions.2"),
    ).toBe(true);
  }
});
