...
- **`publish:auth`** starts a loopback server on `http://localhost:8788/callback`, opens the consent URL, captures the `code`, and exchanges it for tokens.
  ...
  - **Meta** uses Facebook Login for Business (`response_type=code`, plus `display=page` and `extras={"setup":{"channel":"IG_API_ONBOARDING"}}` — required to trigger the Instagram asset-picker during consent), then exchanges the code → short-lived User token → long-lived User token. Resolving the Page + Instagram Business Account ID tries three tiers in order: (1) `GET /me/accounts` + pick the Page with a populated `instagram_business_account`; (2) a per-Page direct lookup (`GET /{page_id}?fields=instagram_business_account`) as a fallback for accounts that don't populate the aggregate field; (3) if both come up empty, pull the granted Page/IG asset ids straight off `GET /debug_token`'s `granular_scopes` (`extractGrantedIds()`) and fetch that Page directly by id (`fetchPageDetails()`) — `/me/accounts` only enumerates Pages the user *broadly* manages and does not enumerate assets granted through the newer asset-scoped consent flow the `IG_API_ONBOARDING` extras param triggers, which is the case for most accounts set up through the current Meta Business Suite UI. Every Graph call authenticated with a User or Page token (not an app token) also needs an `appsecret_proof` param once the app's "Require app secret" setting is on — `appSecretProof()` computes it. Stores `{ user_access_token, user_token_expires_at, page_id, page_access_token, ig_user_id, last_verified_at }` — no `refresh_token` field, because Meta doesn't issue one for Page tokens. Full root-cause writeup of all of this: `docs/publishing/META_INTEGRATION_SPEC.md`.
