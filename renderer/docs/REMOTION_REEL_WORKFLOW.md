...
1. **Voice** — `bun run voice -- <post-key>` routes by `voice_mode`:
   ...
   - **Consistent voice (casting):** VoxCPM2 zero-shot samples a *random* speaker each run, so we seed the RNG to make it **reproducible** — the same `VOXCPM_SEED` (default `777`, or `--seed=N`) gives the **same speaker** every time. To cast a voice you like (e.g. a lower-pitch male), try a handful of seeds (`bun run voice -- <key> --seed=1`, `--seed=2`, …), then lock the winner via `VOXCPM_SEED`. The denoiser is disabled (`load_denoiser=False`) — plain TTS doesn't need it, and it pulled in a pyarrow native lib that crashes on Windows.
