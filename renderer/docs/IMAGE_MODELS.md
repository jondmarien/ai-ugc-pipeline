...
## Cloud art (no local GPU): `--fal` / `--higgsfield` — and opt-in reel motion: `--motion=`

When you don't want to run a local model, two cloud paths generate the **same** per-slide backgrounds (same prompt contract as ComfyUI art, via `lib/art-slide-prompt.mjs`). **Art and reel motion are decoupled:** `--fal` / `--higgsfield` pick the cloud **art** provider only; cloud **reel motion** (per-beat image-to-video) is separately opted into with `--motion=<provider>`. With a cloud art provider the ComfyUI-only knobs (`--flux1`/`--q6`/`--upscale`/`--ui-format`/`--passes`) are ignored and no `free-comfyui` handoff runs; if art stays local but `--motion=` is cloud, ComfyUI still runs for art and the GPU handoff happens as usual.
...
- `--higgsfield --motion=higgsfield` → Higgsfield art + Higgsfield i2v reel.
- `--motion=higgsfield` (no `--higgsfield`) → local ComfyUI art + Higgsfield i2v reel.

- **Art** writes `public/backgrounds/<prefix>/NN_role.png`, flips each slide to `asset_status: "generated"`, and records a public image URL (`fal_image_url` / `higgsfield_image_url`) so the motion step can animate it. Logs licenses into the post's `asset_licenses`.
- **Reel motion** (`reel:fal` / `reel:higgsfield`) writes `public/video/<prefix>/beat_*.mp4` and sets `beat.video_asset` (CTA beats skipped); the Remotion reel composites these clips. In Higgsfield **CLI mode** the local PNG is handed straight to the CLI (it auto-uploads), so no public hosting is needed; otherwise it falls back to `FAL_PUBLIC_BASE_URL` / `HIGGSFIELD_PUBLIC_BASE_URL` / the stored image URL. The reel `.mp4` itself is **always** rendered locally by Remotion.
...
| `--upscale` | GAN upscale of each background. **When art runs, it's integrated into the art graph** (one generate→4×-upscale→lanczos-downscale-to-canvas pass per slide — the exact node chain in `renderer/comfyui-workflows/flux2_klein_4b_8gb_with_upscale.json`). When art is skipped, a standalone pass (`bun run upscale`) sharpens the existing backgrounds. | Default model `RealESRGAN_x4plus.pth` (BSD-3); `--upscale-model=4x-UltraSharp.pth` / `--upscale-scale=N` (final size = canvas × N). **Auto-downloads** the known `.pth` (RealESRGAN_x4plus from the xinntao GitHub release, 4x-UltraSharp from HF `lokCX/4x-Ultrasharp`) to `COMFYUI_UPSCALE_DIR` (default `E:\ComfyUI\models\upscale_models`) if missing — ComfyUI may need a restart to register a freshly-downloaded model. Warn-skips if it can't be fetched. Logged to `LICENSES.md`. |
| `--ui-format` | Art executes the **version-controlled ComfyUI workflow file** (UI format, `renderer/comfyui-workflows/flux2_klein_4b_8gb.json`, or the `_with_upscale` variant with `--upscale`) instead of the code-built graph: the script converts UI→API format and patches each slide's positive prompt + seed (and the upscale `model_name` + `ImageScale` canvas). | **The file's settings win** (steps/CFG/resolution/sampler) — WYSIWYG: edit the workflow in the ComfyUI web UI, save it over the repo file, and the pipeline runs your edits. Default (no flag) = code-built graph; the files are kept as exact mirrors of `buildGraphFlux2()`/`appendUpscale()` — update them in the same commit when the code graph changes. FLUX.2 only (`--flux1` keeps the code graph). |
