...
const TAIL = tailArg
  ? Math.max(0, parseFloat(tailArg.split("=")[1]) || 0)
  : 0.6;
const key =
  argv.find((a) => !a.startsWith("--")) ?? "2026-06-02_ai-phishing-training";
const post = loadPost(key);
if (!post.video?.enabled) {
  console.log(
    `Post ${post.post_id} has video.enabled=false — nothing to render.`,
  );
  process.exit(0);
}
...
if (audio) {
  ...
  const check = (file?: string) =>
    file ? existsSync(path.join(publicDir, file.replace(/^\//, ""))) : false;
  if (audio.voice_mode !== "none" && !check(audio.voice_file)) {
    console.warn(
      `⚠ voice (${audio.voice_mode}): ${audio.voice_file ?? "no file"} not found in public/ — rendering without narration.`,
    );
    if (audio.voice_mode === "voxcpm2")
      console.warn(`  → generate it: bun run voice -- ${key}`);
    audio.voice_mode = "none";
  }
  if (audio.music_mode !== "none" && !check(audio.music_file)) {
    console.warn(
      `⚠ music (${audio.music_mode}): ${audio.music_file ?? "no file"} not found in public/ — rendering without music.`,
    );
    console.warn(
      `  → drop a commercial-safe track at renderer/public${audio.music_file ?? "/audio/<prefix>/music.mp3"} (see pipeline/media/MUSIC_SFX_GUIDE.md), then re-run.`,
    );
  ...
  console.log(
    `Audio: voice=${hasVoice ? "yes" : "—"}  music=${hasMusic ? "yes" : "—"}`,
  );
    ...
    try {
      ...
      if (Number.isFinite(voiceDur)) {
        if (flags.has("--fit-voice")) {
          ...
          for (const n of post.video.narration ?? []) {
            n.start *= scale;
            n.end *= scale;
          }
          post.video.duration_seconds = target;
          console.log(
            `↳ --fit-voice: voice ~${voiceDur.toFixed(1)}s → reel ${target}s (+${TAIL}s tail), beats×${scale.toFixed(2)}.`,
          );
        } else if (reelDur - voiceDur > 2) {
          console.warn(
            `⚠ narration is ~${voiceDur.toFixed(1)}s but the reel is ${reelDur}s — the last ~${(reelDur - voiceDur).toFixed(0)}s will be silent.`,
          );
          console.warn(
            `  Fix: add --fit-voice (auto-trims + realigns), set video.duration_seconds≈${Math.ceil(voiceDur) + 1}, or add a music bed.`,
          );
        }
      }
    } catch {
...
const propsFile = path.join(
  os.tmpdir(),
  `ai-ugc-reel-props-${post.slug}-${post.date}.json`,
);
writeFileSync(propsFile, JSON.stringify({ post }), "utf8");

console.log(
  `Rendering reel ${post.video.export_name} (${post.video.duration_seconds}s @ ${post.video.fps}fps, captions: ${post.video.caption_mode})…`,
);
