...
const srcDir =
  !srcArg || srcArg === "flux2" || srcArg === "_flux2"
    ? path.join(RENDERER, "public", "backgrounds", `${prefix}_flux2`)
    : srcArg === "higgsfield"
      ? path.resolve(
          path.join(RENDERER, "public", "backgrounds", `${prefix}_higgsfield`),
        )
      : path.resolve(srcArg);
...
const srcImgs = readdirSync(srcDir)
  .filter((f) => IMG.test(f))
  .sort();
if (!srcImgs.length) {
  console.error(`No images (.png/.jpg/.webp) in ${srcDir}`);
  process.exit(1);
}

const targets = post.slides.filter(
  (s) => flags.has("--all") || s.role !== "cover",
);
...
function pick(slide) {
  ...
  return (
    ...
    srcImgs.find(
      (f) =>
        !used.has(f) && f.toLowerCase().includes(slide.role.replace("_", "-")),
    ) ||
...
targets.forEach((slide, idx) => {
  ...
  if (!src) {
    console.warn(
      `⚠ slide ${slide.slide} (${slide.role}): no source image matched — left as ${slide.asset_status}`,
    );
    return;
  }
  used.add(src);
