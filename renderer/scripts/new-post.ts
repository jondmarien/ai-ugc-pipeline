...
const captionsFlag =
  flagArgs.find((a) => a.startsWith("--captions="))?.split("=")[1] ??
  "highlight";
const captionMode = (CAPTION_MODES as readonly string[]).includes(captionsFlag)
  ? captionsFlag
  : "highlight";
...
const voiceFlag =
  flagArgs.find((a) => a.startsWith("--voice="))?.split("=")[1] ?? "voxcpm2";
const musicFlag =
  flagArgs.find((a) => a.startsWith("--music="))?.split("=")[1] ?? "none";
const voiceMode = (VOICE_MODES as readonly string[]).includes(voiceFlag)
  ? voiceFlag
  : "voxcpm2";
const musicMode = (MUSIC_MODES as readonly string[]).includes(musicFlag)
  ? musicFlag
  : "none";
const THEMES = [
  "offensive",
...
const themeFlag =
  flagArgs.find((a) => a.startsWith("--theme="))?.split("=")[1] ?? "";
...
const styleFusionFlag =
  flagArgs
    .find((a) => a.startsWith("--style-fusion="))
    ?.split("=")
    .slice(1)
    .join("=")
    .trim() ?? "";
...
const SLIDES_DEFAULT = 8,
  SLIDES_MIN = 3,
  SLIDES_MAX = 20;
const slidesFlag = flagArgs
  .find((a) => a.startsWith("--slides="))
  ?.split("=")[1];
const slideCount =
  slidesFlag === undefined ? SLIDES_DEFAULT : Number(slidesFlag);

const multipleCaptionsArg = flagArgs.find((a) =>
  a.startsWith("--multiple-captions"),
);
...
function usageAndExit(msg?: string): never {
  ...
  console.error(
    `  theme  ∈ offensive (red) | defensive (blue) | hacking (green) | purple-team (purple) | ai (generic AI, orange)  — optional; defaults from pillar`,
  );
  console.error(
    `  example: bun run new -- 2026-06-13 ai-agent-permissions model_security --theme=defensive\n`,
  );
  process.exit(1);
}
...
if (!/^\d{4}-\d{2}-\d{2}$/.test(date))
  usageAndExit(`date "${date}" must be YYYY-MM-DD.`);
if (!/^[a-z0-9-]+$/.test(slug))
  usageAndExit(`slug "${slug}" must be lowercase kebab-case (a-z 0-9 -).`);
if (!PILLARS.includes(pillarArg as Pillar))
  usageAndExit(`pillar "${pillarArg}" is not valid.`);
if (!(CAPTION_MODES as readonly string[]).includes(captionsFlag))
  usageAndExit(
    `--captions "${captionsFlag}" must be one of: ${CAPTION_MODES.join(", ")}.`,
  );
if (!(VOICE_MODES as readonly string[]).includes(voiceFlag))
  usageAndExit(
    `--voice "${voiceFlag}" must be one of: ${VOICE_MODES.join(", ")}.`,
  );
if (!(MUSIC_MODES as readonly string[]).includes(musicFlag))
  usageAndExit(
    `--music "${musicFlag}" must be one of: ${MUSIC_MODES.join(", ")}.`,
  );
if (themeFlag && !(THEMES as readonly string[]).includes(themeFlag))
  usageAndExit(`--theme "${themeFlag}" must be one of: ${THEMES.join(", ")}.`);
if (
...
const theme = (
  (THEMES as readonly string[]).includes(themeFlag)
    ? themeFlag
    : pillarTheme[pillar]
) as Theme;
...
> = {
  cover: {
    ...
    copy: "YOUR COVER HOOK GOES HERE",
    subline: "One-line promise of what they'll learn.",
    cta: "SWIPE →",
    status: "needed",
  ...
  context: {
    kicker: "WHAT CHANGED",
    copy: "What happened, or the pattern to notice.",
    subline: "Keep it plain-language.",
  ...
  risk: {
    ...
    copy: "Why this matters to defenders.",
    subline: "Security impact, not hype.",
  ...
  mechanism: {
    kicker: "HOW IT WORKS",
    copy: "Safe, high-level mechanism.",
    subline: "No payloads or exploit steps.",
  ...
  failure_point: {
    ...
    copy: "The people / process / tooling gap.",
    subline: "Where this slips through today.",
  ...
  defense: {
    ...
    copy: "One concrete control or review step.",
    subline: "Something they can do this week.",
  ...
  takeaway: {
    kicker: "TAKEAWAY",
    copy: "One memorable, save-worthy line.",
    subline: "",
  ...
  cta: {
    kicker: "YOUR MOVE",
    copy: "A specific question for the comments?",
    subline: "Save this for your next review.",
    cta: "COMMENT + SAVE",
  ...
  point: {
    kicker: "POINT",
    copy: "One more point worth making.",
    subline: "Keep it specific and concrete.",
...
const slides = sequence.map((role, i) => {
  ...
  return {
    ...
    subline: s.subline,
    visual_direction:
      "cinematic, dark, single accent glow; text-free background; protected lower-third.",
    // Rich per-slide art prompt — used directly by `bun run art`. Replace [the topic].
    visual_prompt: s.vp,
    background_asset:
      role === "cover" ? `/backgrounds/${prefix}_cover.png` : "",
    // cover uses 'needed' (swap to 'existing' once you add the PNG); inners are procedural CSS.
    asset_status: s.status ?? "procedural",
    cta: s.cta ?? "",
    notes:
      role === "cover"
        ? "Add a 1080×1350 text-free PNG to renderer/public/backgrounds/ then set asset_status to 'existing'."
        : "",
  };
});
...
const anchorPlan: Array<{
  ...
  {
    ref: roleRef("takeaway"),
    purpose: "takeaway",
    motion: "static high contrast",
  },
  { ref: slideCount, purpose: "cta", motion: "end card" },
];
const seenRefs = new Set<number>();
const anchors = anchorPlan.filter(
  (a): a is { ref: number; purpose: string; motion: string } => {
    if (a.ref == null || seenRefs.has(a.ref)) return false;
    seenRefs.add(a.ref);
    return true;
  },
);
const beatBound = (i: number) =>
  Math.round((i * reelDuration) / anchors.length);
const narration = anchors.map((a, i) => ({
  start: beatBound(i),
  end: beatBound(i + 1),
  text: `TODO ${a.purpose} line.`,
}));
...
const post = {
  ...
  score: {
    ...
    novelty: 3,
    visual_drama: 3,
    defender_usefulness: 3,
    total: 15,
  },
  canvas: {
    width: canvas.carousel.width,
    height: canvas.carousel.height,
    safe_margin: canvas.carousel.safeMargin,
  },
  brand: {
  ...
  upload_package: {
    ...
    expected_files: [
      ...slides.map(
        (s) =>
          `${prefix}_${String(s.slide).padStart(2, "0")}_${ROLE_FILENAME[s.role]}.png`,
      ),
      "caption.txt",
      "alt_text.txt",
      "sources.md",
  ...
  ...(multipleCaptionsFlag
    ? {
        slide_captions: slides.map(
          (s) =>
            `TODO ${s.role} Instagram caption: short per-slide caption distinct from on_slide_copy.`,
        ),
      }
    : {}),
  slides,
  caption:
    "TODO: hook restated · what happened · why it matters · defender takeaway · question.\n\nFollow for AI security breakdowns without the fake panic.",
  ...
  alt_text: slides.map(
    (s) =>
      `TODO ${s.role} alt text: restate this slide's message, then one brief clause of visual scene.`,
  ),
  sources: [
    {
      source: "TODO source name",
      link: "https://example.com",
      supports: "Which claim this supports.",
      confidence: "medium",
      claim_tag: "scenario",
    },
  ],
  ...
  video: {
    ...
    audio: {
      ...
      voice_file:
        voiceMode === "none" ? undefined : `/audio/${prefix}/voice.wav`,
      voice_gain_db: 0,
      music_mode: musicMode,
      music_file:
        musicMode === "none" ? undefined : `/audio/${prefix}/music.mp3`,
      music_gain_db: -18,
    },
    narration,
    beats,
...
console.log(
  `  pillar: ${pillar}  ·  theme: ${theme} (${themeDef.name})  ·  ${slideCount} slides  ·  reel enabled (${reelDuration}s)  ·  captions: ${captionMode}${multipleCaptionsFlag ? "  ·  multiple_captions: on" : ""}\n`,
);
console.log("Next:");
console.log(
  `  1. Edit the file, replacing every TODO (copy, caption, sources, alt text).`,
);
console.log(
  `  2. (optional) Add a cover image to renderer/public/backgrounds/${prefix}_cover.png and set slide 1 asset_status to "existing".`,
);
console.log(`  3. bun run validate -- ${prefix}`);
console.log(
  `  4. bun run export -- ${prefix} && bun run package -- ${prefix} && bun run reel -- ${prefix}\n`,
);
