...
function findPost(key) {
  if (!key) {
    console.error("Usage: node scripts/voice-http.mjs <post-key>");
    process.exit(1);
  }
  const f = readdirSync(POSTS).find(
    (x) => x.endsWith(".json") && x.includes(key),
  );
  if (!f) {
...
const narration = (post.video?.narration ?? [])
  .map((n) => n.text?.trim())
  .filter(Boolean);
if (!narration.length) {
  console.error("Post has no video.narration[] to synthesize.");
  process.exit(1);
}
...
console.log(
  `HTTP TTS → ${BASE}/audio/speech  (model=${MODEL}, voice=${VOICE}, format=${FORMAT})`,
);
console.log(
  `Script (${script.length} chars): ${script.slice(0, 120)}${script.length > 120 ? "…" : ""}\n`,
);
...
try {
  ...
  console.error(
    `✗ Could not reach ${BASE}/audio/speech — is your TTS server running?`,
  );
  console.error(
    `  Try Kokoro-FastAPI:  docker run -p 8880:8880 ghcr.io/remsky/kokoro-fastapi-cpu  (or -gpu)`,
  );
  console.error(
    `  Then: TTS_BASE_URL=http://localhost:8880/v1 bun run voice -- ${key}`,
  );
  console.error(
    `  (LM Studio on :1234 does NOT serve /v1/audio/speech as of 2026.)\n  ${e}`,
  );
  process.exit(1);
}
if (!res.ok) {
...
console.log(
  "  Reminder: label AI-generated audio; confirm the voice/model license allows commercial use.",
);
