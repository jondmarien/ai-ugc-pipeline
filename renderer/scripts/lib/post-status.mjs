...
const STATUS_RE =
  /^(\s*)"status": "(?:draft|approved|generated|upload_ready)"/m;
...
export function readStatus(key) {
  try {
    const m = readFileSync(path.join(POSTS, toFile(key)), "utf8").match(
      /"status": "(draft|approved|generated|upload_ready)"/,
    );
    return m ? m[1] : null;
  } catch {
    return null;
...
export function setStatus(
  ...
  if (onlyFrom && !onlyFrom.includes(old))
    return { key: k, old, new: newStatus, changed: false, reason: "skipped" };
  if (old === newStatus)
    return { key: k, old, new: newStatus, changed: false, reason: "same" };
  if (!STATUS_RE.test(s))
    return { key: k, old, changed: false, reason: "no-status-line" };
  if (!dryRun)
    writeFileSync(
      fp,
      s.replace(STATUS_RE, (_, ind) => `${ind}"status": "${newStatus}"`),
    );
