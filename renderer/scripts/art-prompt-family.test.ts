...
const slide = { slide: 1, role: "cover", visual_prompt: "a cracked kernel ring on dark metal" };
const ctx = { accentHex: "#ef4444", accentName: "red", topic: "linux lpe", mood: "tense", styleFusion: "" };

test("buildSlidePrompt stays the FLUX house string (spec → composeFluxPrompt)", () => {
  expect(buildSlidePrompt(slide, ctx)).toBe(composeFluxPrompt(buildPromptSpec(slide, ctx)));
});

test("flux family = rich prose (with #hex) + positive exclusions, empty negative", () => {
  const { prompt, negative } = composePromptForFamily(buildPromptSpec(slide, ctx), "flux");
...
test("natural families (soul/seedream/gpt) lead with exclusions and drop #hex", () => {
  for (const fam of ["soul", "seedream", "gpt"]) {
    const { prompt, negative } = composePromptForFamily(buildPromptSpec(slide, ctx), fam);
    expect(prompt.toLowerCase().startsWith("pure abstract background")).toBe(true);
