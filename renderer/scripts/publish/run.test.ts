...
test("planPublish throws when the post is a draft", () => {
  expect(() => planPublish(key, ["youtube"], { status: "draft", state: {}, force: false })).toThrow(/generated/i);
});
...
test("planPublish rejects an approved-but-unrendered post (publish requires generated)", () => {
  expect(() => planPublish(key, ["youtube"], { status: "approved", state: {}, force: false })).toThrow(/generated/i);
});
...
test("planPublish skips an already-published platform unless force", () => {
  ...
  const p1 = planPublish(key, ["youtube", "tiktok"], { status: "generated", state, force: false });
  expect(p1.toRun).toEqual(["tiktok"]);
  expect(p1.skipped).toEqual(["youtube"]);

  const p2 = planPublish(key, ["youtube", "tiktok"], { status: "generated", state, force: true });
...
test("planPublish returns human-readable summary lines naming each platform + action", () => {
  const state = { youtube: { platform: "youtube", status: "published", at: 1 } };
  const p = planPublish(key, ["youtube", "tiktok"], { status: "generated", state, force: false });
...
test("planPublish --force does NOT bypass the gate (approved still throws under force)", () => {
  expect(() => planPublish(key, ["youtube"], { status: "approved", state: {}, force: true })).toThrow(/generated/i);
  expect(() => planPublish(key, ["youtube"], { status: "draft", state: {}, force: true })).toThrow(/generated/i);
});
