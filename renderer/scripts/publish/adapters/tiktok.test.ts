...
import { makeTiktokAdapter, pickPrivacy, shapeTiktokResult } from "./tiktok";
...
test("pickPrivacy throws a clear privacy-mismatch error when not allowed", () => {
  expect(() => pickPrivacy(creatorInfo as any, "PUBLIC_TO_EVERYONE")).toThrow(
    /privacy/i,
  );
});

test("shapeTiktokResult maps a complete status response to a published AdapterResult", () => {
  const r = shapeTiktokResult(
    statusResp as any,
    "SELF_ONLY",
...
test("adapter runs creator_info -> init -> upload -> poll and includes Content-Range + Content-Type on the PUT", async () => {
  ...
  const fakeFetch = (async (url: any, init: any) => {
    ...
    if (u.includes("/creator_info/query/")) {
      seen.push("creator_info");
      return new Response(JSON.stringify(creatorInfo), { status: 200 });
    }
    if (u.includes("/video/init/")) {
      seen.push("init");
      return new Response(JSON.stringify(initResp), { status: 200 });
    }
    if (init?.method === "PUT") {
      seen.push("put");
    ...
    if (u.includes("/status/fetch/")) {
      seen.push("status");
      return new Response(JSON.stringify(statusResp), { status: 200 });
    }
