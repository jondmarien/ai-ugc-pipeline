...
async function uploadImage(absPath) {
  ...
  const r = await fetch(`${URL_BASE}/upload/image`, {
    method: "POST",
    body: form,
  });
...
function buildGraph(imageRef, targetW, targetH) {
  return {
    ...
    5: {
      ...
      inputs: { filename_prefix: "upscale_pipeline/up", images: ["4", 0] },
    },
  };
}
...
async function runGraph(graph) {
  ...
  while (Date.now() < deadline) {
    ...
    if (entry.status?.status_str === "error")
      throw new Error(
        `ComfyUI error: ${JSON.stringify(entry.status?.messages)}`,
      );
    const out =
      entry.outputs &&
...
const bgAbs = (s) =>
  s.background_asset
    ? path.join(RENDERER, "public", s.background_asset.replace(/^[\\/]+/, ""))
    : null;
...
if (!targets.length) {
  console.log("No existing slide backgrounds to upscale (run art first).");
  process.exit(0);
}

if (DRY) {
  console.log(
    `DRY RUN — would upscale ${targets.length} slide background(s) with ${MODEL} → ${targetW}x${targetH} (canvas ×${SCALE}).`,
  );
  for (const s of targets)
    console.log(`  slide ${s.slide} (${s.role}) ← ${s.background_asset}`);
  process.exit(0);
}
...
let available = [];
try {
  const info = await fetch(`${URL_BASE}/object_info/UpscaleModelLoader`).then(
    (r) => r.json(),
  );
...
if (
  ...
  if (got) {
    try {
      const info2 = await fetch(
        `${URL_BASE}/object_info/UpscaleModelLoader`,
      ).then((r) => r.json());
      available =
        info2?.UpscaleModelLoader?.input?.required?.model_name?.[0] ??
        available;
    } catch {
      /* keep prior list */
    }
  ...
  if (!available.includes(MODEL)) {
    console.warn(
      `⚠ upscale model "${MODEL}" not available to ComfyUI${got ? " yet — downloaded to disk, but ComfyUI may need a restart/refresh to register it" : ""} — skipping upscale.`,
    );
    console.warn(`  Available now: ${available.join(", ") || "(none)"}`);
    if (!got && !MODEL_SOURCES[MODEL])
      console.warn(
        `  Unknown model; drop the .pth in ${COMFY_UPSCALE_DIR} and re-run with --upscale-model=<name>.`,
      );
...
console.log(
  `Upscaling ${targets.length} background(s) @ ${URL_BASE} · ${MODEL} → ${targetW}x${targetH} (canvas ×${SCALE})`,
);
let ok = 0;
