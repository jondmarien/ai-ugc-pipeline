...
function die(msg) {
  ...
  console.error(
    `  per-idea options: "idea::pillar::captions=word::style_fusion=X meets Y"`,
  );
  console.error(
    `  pillars: ${PILLARS.join(" | ")}   ·   captions: ${MODES.join(" | ")}`,
  );
  console.error(
    `  example: bun run draft-week -- "voice clone fraud::offensive_ai" "RAG leaks::model_security::captions=highlight" "shadow AI::governance"\n`,
  );
  process.exit(1);
}
...
if (claudeCheck.status !== 0)
  die(
    "`claude` CLI not found on PATH. Use the interactive /draft-week command instead.",
  );
...
const parsed = ideas.map((raw) => {
  ...
  for (const p of parts.slice(1)) {
    ...
    else if (p.startsWith("captions=") && MODES.includes(p.split("=")[1]))
      captions = p.split("=")[1];
    else if (p.startsWith("style_fusion="))
      styleFusion = p.slice("style_fusion=".length).trim();
  }
  return {
    text,
    pillar,
...
const list = parsed
  .map(
    (p, i) =>
      `  ${i + 1}. "${p.text}"  [pillar: ${p.pillar ?? "you choose — diversify"}; captions: ${p.captions}${p.styleFusion ? `; style_fusion: "${p.styleFusion}"` : ""}]`,
  )
  .join("\n");
...
const keys = [
  ...out.matchAll(/POST_KEY=([0-9]{4}-[0-9]{2}-[0-9]{2}_[a-z0-9-]+)/g),
].map((m) => m[1]);
...
for (const k of uniq) {
  ...
  console.log(
    `  ${ok ? "✓" : "⚠ missing JSON"}  ${k}  →  pipeline/renders/${k}/`,
  );
}
console.log(
