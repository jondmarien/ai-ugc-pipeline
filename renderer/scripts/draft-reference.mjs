...
const takeawayOf = (p) =>
  stripAccent(slideRole(p, "takeaway")?.on_slide_copy || "");
...
const nounsOf = (str) =>
  ((str || "").toLowerCase().match(/[a-z][a-z-]{2,}/g) || []).filter(
    (w) => !STOP.has(w),
  );
...
for (const p of posts)
  for (const s of p.slides || [])
    for (const w of nounsOf(s.visual_prompt))
      motif.set(w, (motif.get(w) || 0) + 1);
const overusedMotifs = [...motif.entries()]
  .filter(([, c]) => c >= 3)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 22);
...
const ANGLES = {
  "indirect prompt injection":
    /prompt[ -]?inject|injected instructions|steer(s|ed)? the agent|poisoned (?:page|doc)/i,
  "data exfiltration / leakage":
    /exfil|leak|data (?:moves|out|both ways)|drive ?mount/i,
  "credential & token scope":
    /credential|token|oauth|\bscope|api key|long-lived/i,
  "supply chain / dependency trust":
    /supply[ -]?chain|dependency|plugin|package|mcp server|skills? execute/i,
  "cost & resource abuse":
    /\bcost\b|crypto-?min|resource abuse|billable|gpu (?:abuse|bill)|cost-?bomb/i,
  "identity & authentication":
    /identity|authenticat|proof of identity|second channel|call ?back|code word|voice/i,
  "autonomy & blast radius":
    /blast radius|long-running|autonom|hundreds of turns|goal drift|run for hours/i,
  "auditability & logging":
    /\baudit|\blog(s|ging|ged)?\b|forensic|trail|read the (?:tool|stream)/i,
  "governance, approval & human-in-the-loop":
    /policy|governance|\bapprov|dual auth|human (?:in|review)|gate (?:it|what)/i,
  "least privilege / sandboxing":
    /least privilege|sandbox|isolat|scoped (?:key|account)|throwaway vm|disposable/i,
  "myth-busting the hype":
    /not (?:magic|theoretical|a step-change)|overhyped|\bhype\b|\bmyth|force multiplier|lower floor/i,
};
const angleHits = Object.fromEntries(Object.keys(ANGLES).map((k) => [k, []]));
for (const p of posts) {
  const blob = [p.core_claim, takeawayOf(p), p.caption].join("  ");
  for (const [k, re] of Object.entries(ANGLES))
    if (re.test(blob)) angleHits[k].push(p.slug || p.post_id || "?");
}
const used = Object.entries(angleHits)
  .filter(([, a]) => a.length)
...
L.push(
  `Use this as a NOT-list. The next post should NOT reuse these hooks, visual objects, or the same takeaway angle. Vary deliberately.`,
);
L.push("");
...
L.push(
  overusedOpeners.length
    ? overusedOpeners.map(([o, c]) => `  - "${o}…" ×${c}`).join("\n")
    : "  (none repeated yet)",
);
L.push(
  `  Hook formulas to rotate through: contradiction ("Everyone thinks X. Actually Y."), command ("Stop X. Start Y."), a number/stat, a named scenario ("Someone used AI to…"), a blunt question, a myth flip.`,
);
L.push("");
L.push(
  `## Overused visual motifs (do NOT build the new prompts from these — invent fresh objects)`,
);
L.push(
...
L.push("");
L.push(
  `## Defender-takeaway ANGLE coverage (rotate — not every AI-tool post is "indirect prompt injection / it's unsafe")`,
);
...
L.push("");
L.push(
  `Rule of thumb: a reader scrolling your feed should not see the same hook shape, the same dark-padlock-and-key imagery, or the same "this is unsafe, use least privilege" ending on post after post. Same standards (sourced, defender takeaway), different surface every time.`,
);
