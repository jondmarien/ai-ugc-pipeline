...
`/draft-post` also takes first-class pipe options, in any order after the pillar: `theme=offensive|defensive|hacking|purple|ai` (brand colour: red / blue / green / purple-team / generic-AI orange), `voice=none|voxcpm2|voxcpm2-0.5b|http`, `captions=…`. **Research is a loop** (landscape scan → triangulate ≥2 independent sources → tier each claim `[Verified]/[Emerging]/[Scenario]` → hard gates: no fabricated URLs, no uncited victims), then the **humanizer** pass rewrites the copy in-voice without touching facts.
