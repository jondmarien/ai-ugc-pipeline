...
export function summarize(items: MediaItem[]) {
  ...
  return {
    ...
    totalReelViews: sum((m) =>
      m.media_type === "VIDEO" ? (m.insights.views ?? 0) : 0,
    ),
    savesRate: reachSum
      ? (sum((m) => m.insights.saved ?? 0) / reachSum) * 100
      : 0,
    sharesRate: reachSum
...
export function reelWatchStats(items: MediaItem[]) {
  const reels = items.filter(
    (m) =>
      m.media_type === "VIDEO" && m.insights.ig_reels_avg_watch_time != null,
  );
  if (!reels.length) return null;
  const best = [...reels].sort(
    (a, b) =>
      (b.insights.ig_reels_avg_watch_time ?? 0) -
      (a.insights.ig_reels_avg_watch_time ?? 0),
  )[0];
  return {
    avgWatchMs:
      reels.reduce((a, m) => a + (m.insights.ig_reels_avg_watch_time ?? 0), 0) /
      reels.length,
    totalWatchMs: reels.reduce(
      (a, m) => a + (m.insights.ig_reels_video_view_total_time ?? 0),
      0,
    ),
...
export function dayOfWeekBuckets(
  ...
  for (const m of items) {
    ...
    cur.avgEngagement =
      (cur.avgEngagement * cur.count + engagementRate(m)) / (cur.count + 1);
    cur.count += 1;
    out[d] = cur;
  }
  return out;
...
export function hourBuckets(
  ...
  const label = (h: number) =>
    h < 6 ? "00-06" : h < 12 ? "06-12" : h < 18 ? "12-18" : "18-24";
  const out: Record<string, { count: number; avgEngagement: number }> = {};
  for (const m of items) {
...
export function hashtagStats(
  items: MediaItem[],
): { tag: string; count: number; avgEngagement: number }[] {
