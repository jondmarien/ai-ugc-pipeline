...
import { type HooksMeta, PILLARS } from "../hooks/HookVault";
...
export function Trending() {
  ...
  const items = (trends.data?.items ?? []).filter(
    (i) => tags[i.url] !== "skip",
  );
    ...
    <>
      ...
      {!trends.loading && items.length === 0 && (
        <EmptyState
          ...
          hint="Enable sources in data/sources.json and press Refresh."
        />
      )}
      {items.map((i) => (
        <Panel key={i.url}>
          ...
          >
            <div>
              ...
              <div className="meta-caps">
                {i.sourceLabel} ·{" "}
                {new Date(i.publishedAt).toISOString().slice(0, 10)}
              </div>
            </div>
