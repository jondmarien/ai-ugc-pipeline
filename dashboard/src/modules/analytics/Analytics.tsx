...
export function Analytics() {
  ...
  const last20 = useMemo(
    () => [...items].sort(sortFn.recent).slice(0, 20).reverse(),
    [items],
  );
  ...
  if (!media.loading && !items.length)
    return (
      <>
        ...
        />
        <EmptyState
          title="NO IG DATA YET"
          hint="Set up dashboard/.env per the spec prerequisites, then press Refresh."
        />
      </>
    );
  ...
  return (
    <>
      <h1 className="page-title">
        Analytics{account.data ? ` · @${account.data.username}` : ""}
      </h1>
      <StalenessBanner
        label="IG DATA"
        error={media.error}
      ...
      {watch && (
        <div className="grid cols-4" style={{ marginBottom: 16 }}>
          <StatCard
            ...
            value={`${(watch.avgWatchMs / 1000).toFixed(1)}s`}
          />
          <StatCard
            label="TOTAL WATCH TIME"
            value={`${(watch.totalWatchMs / 60000).toFixed(0)}m`}
          />
          <StatCard
            label="REEL VIEWS"
            value={watch.totalViews.toLocaleString()}
          />
          <StatCard
            label="BEST WATCH-TIME REEL"
            value={`${((watch.best.insights.ig_reels_avg_watch_time ?? 0) / 1000).toFixed(1)}s`}
      ...
      <div className="grid cols-3" style={{ marginBottom: 16 }}>
        ...
        <Panel>
          <div className="meta-caps">
            ENGAGEMENT · LAST {last20.length} POSTS
          </div>
          <Bars
            values={last20.map((m) => m.like_count + m.comments_count)}
      ...
      <div className="grid cols-3" style={{ marginBottom: 16 }}>
        ...
        <Panel>
          ...
          <Bars
            values={Object.values(days).map((v) => v.avgEngagement)}
            labels={Object.entries(days).map(
              ([d, v]) =>
                `${d} · ${v.avgEngagement.toFixed(1)}% · ${v.count} posts`,
            )}
            height={64}
          />
          <div className="meta-caps">{Object.keys(days).join(" · ")}</div>
          <Bars
            values={Object.values(hours).map((v) => v.avgEngagement)}
            labels={Object.entries(hours).map(
              ([h, v]) =>
                `${h} · ${v.avgEngagement.toFixed(1)}% · ${v.count} posts`,
            )}
            height={64}
          />
      ...
      {tags.length > 0 && (
        <Panel>
          <div className="meta-caps" style={{ marginBottom: 8 }}>
            HASHTAGS (USED 2+)
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {tags.map((t) => (
              <span key={t.tag} className="chip">
                {t.tag} · {t.avgEngagement.toFixed(1)}% · {t.count}×
              </span>
            ))}
          </div>
        </Panel>
  ...
  return (
    <div style={{ padding: "6px 0" }}>
      ...
      <div className="meta-caps">
        ...
        <span
          className="chip"
          style={{ color: tierColor[tierFor(rate)], marginLeft: 8 }}
        >
          {rate.toFixed(1)}%
        </span>
      </div>
