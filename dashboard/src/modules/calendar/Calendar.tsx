...
export function CalendarView() {
  ...
  const schedule = useApi<{ items: ScheduleItem[] }>(
    "/api/state/schedule.json",
  );
  const now = new Date();
  ...
  const caption = useApi<string>(
    openDir
      ? `/api/repo/renders/${encodeURIComponent(openDir)}/caption.txt`
      : "/api/health",
  );
  const sources = useApi<string>(
    openDir
      ? `/api/repo/renders/${encodeURIComponent(openDir)}/sources.md`
      : "/api/health",
  );
  ...
  const byDate = useMemo(() => {
    ...
    for (const r of renders.data ?? []) {
      ...
      map.set(r.date, [
        ...(map.get(r.date) ?? []),
        { label: r.slug, dir: r.dirName, kind: "rendered", platform: null },
      ]);
    }
    return map;
  ...
  const platformColor: Record<string, string> = {
    instagram: "var(--theme-purple-team)",
    tiktok: "var(--theme-hacking)",
    linkedin: "var(--theme-defensive)",
  };
  const slotColor = (s: { kind: string; platform: string | null }) =>
    s.kind === "rendered"
      ? "var(--muted)"
      : (platformColor[s.platform ?? ""] ?? "var(--accent)");
            ...
            {(byDate.get(date) ?? []).map((s, i) => (
              <button
                ...
                style={{
                  ...
                  display: "block",
                  marginTop: 4,
                  opacity: s.kind === "posted" ? 0.6 : 1,
                }}
                onClick={() => s.dir && setOpenDir(s.dir)}
              >
                {s.label.slice(0, 18)}
              </button>
            ))}
        ...
        <Panel>
          ...
          >
            {openPkg?.slides.map((f) => (
              <img
                key={f}
                src={`/api/repo/renders/${encodeURIComponent(openDir)}/slide/${encodeURIComponent(f)}`}
          ...
          <pre style={{ whiteSpace: "pre-wrap", fontSize: 12 }}>
            {typeof caption.data === "string" ? caption.data : ""}
          </pre>
          <div className="meta-caps">SOURCES</div>
          <pre style={{ whiteSpace: "pre-wrap", fontSize: 12 }}>
            {typeof sources.data === "string" ? sources.data : ""}
          </pre>
        </Panel>
      )}
    </>
