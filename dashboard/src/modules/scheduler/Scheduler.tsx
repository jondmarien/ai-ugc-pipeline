...
import { type ScheduleItem, transition } from "../../lib/schedule";
...
export function Scheduler() {
  ...
  const queue = (pkg: RenderPkg) => {
    ...
    persist([
      ...items,
      {
        id: `${pkg.dirName}-${Date.now()}`,
        renderDir: pkg.dirName,
        date,
        time,
        platforms: ["instagram"],
        status: "queued",
    ...
    persist(
      items.map((i) =>
        i.id === id
          ? {
              ...i,
              platforms: i.platforms.includes(p)
                ? i.platforms.filter((x) => x !== p)
                : [...i.platforms, p],
            }
          : i,
      ),
  ...
  const unqueued = (renders.data ?? []).filter(
    (r) =>
      !items.some((i) => i.renderDir === r.dirName && i.status === "queued"),
  );
  ...
  return (
    <>
      ...
      <p className="meta-caps">
        PLANNING QUEUE ONLY. NOTHING HERE PUBLISHES ANYWHERE.
      </p>
      <Panel>
        ...
        {unqueued.length === 0 && (
          <span className="meta-caps">
            ALL PACKAGES QUEUED OR NONE RENDERED.
          </span>
        )}
        {unqueued.map((r) => (
          <div
      ...
      {items.length === 0 ? (
        <EmptyState
          title="QUEUE EMPTY"
          hint="Queue a rendered package above to plan its posting slot."
        />
      ) : (
        items.map((i) => (
          <Panel key={i.id}>
            ...
            >
              <div>
                ...
                <div className="meta-caps">
                  {i.date} {i.time} · {i.status.toUpperCase()}
                  {i.postedAt ? ` AT ${i.postedAt.slice(0, 16)}` : ""}
                </div>
              </div>
