    ...
    out.push({
      title:
        typeof e.title === "object"
          ...
          : String(e.title ?? ""),
      url: link?.["@_href"] ?? "",
      publishedAt: Date.parse(e.updated ?? e.published ?? "") || 0,
      sourceLabel,
    });
  }
...
export async function getTrends(force = false) {
  const sources = (readState("sources.json") as SourceEntry[]).filter(
    (s) => s.enabled,
  );
  return fetchWithCache(
    ...
    TRENDS_CACHE_DIR,
    { maxAgeMs: 3_600_000, force },
  ); // trends go stale faster: 1h
}
