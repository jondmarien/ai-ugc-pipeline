...
describe("parseFeed", () => {
  test("parses RSS 2.0", () => {
    const items = parseFeed(
      fs.readFileSync(path.join(fx, "rss-sample.xml"), "utf8"),
      "Fixture",
    );
    expect(items).toHaveLength(2);
    expect(items[0]).toEqual({
      title: "AI agent CVE drops",
      url: "https://example.com/1",
      publishedAt: Date.parse("Sat, 06 Jun 2026 10:00:00 GMT"),
      sourceLabel: "Fixture",
    });
  });
  test("parses Atom", () => {
    const items = parseFeed(
      fs.readFileSync(path.join(fx, "atom-sample.xml"), "utf8"),
      "Atom",
    );
    expect(items).toHaveLength(2);
