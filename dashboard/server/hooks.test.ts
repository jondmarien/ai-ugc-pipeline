...
import { aggregateHooks, normalizeHook, parseCaptionBankHooks } from "./hooks";
...
describe("normalizeHook", () => {
  test("lowercases, collapses whitespace, strips trailing punctuation", () => {
    expect(normalizeHook("  Your AI agent  reads EVERYTHING. ")).toBe(
      "your ai agent reads everything",
    );
  });
});
...
describe("parseCaptionBankHooks", () => {
  test("takes second cell of body rows in section 1 only", () => {
    const hooks = parseCaptionBankHooks(
      path.join(fx, "CAPTION_BANK_fixture.md"),
    );
...
describe("aggregateHooks", () => {
  test("merges sources, dedupes by normalized text, counts uses from posts", () => {
    const rows = aggregateHooks({
      posts: [
        {
          coverHook: "Stop pasting production logs into chatbots",
          slug: "a",
          date: "2026-06-01",
        },
        {
      ...
      ingested: [
        {
          coverHook: "Steal my fixture system",
          fileName: "x.md",
          date: "2026-06-07",
        },
      ],
