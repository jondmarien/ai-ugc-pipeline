...
| --- | --- | --- | --- |
| 1 | Cover hook | 8 words max, concrete and countable (a number, a named object, a verb). Its only job is stopping the scroll; the subline carries any qualifier. e.g. “AI PHISHING JUST CROSSED A LINE” Optional `closer`: a two-clause pain-point line at the base of the cover, setup + accent-marked (`[[…]]`) consequence, e.g. “your prompts are fine. `[[the injection isn’t in the prompt.]]`” — keep it tied to the post’s actual claim, not a growth-guru attention-grab. | **≤8 words** on the body line; subline **≤30 words** |
