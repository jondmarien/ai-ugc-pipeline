...
1. Use `ai-cybersecurity-ugc-carousel` to write the post (default 8 slides, or N if `::slides=N` was given — `cover` first, `cta` last, `takeaway` at N−1, middle from the named roles then generic `point` slides) + caption + a bracketed topic list (NOT hashtags — plain topics in the `hashtags` field) + question (house voice).
...
4. `cd renderer && npm run new -- <date> <slug> <pillar> [--slides=N] --captions=<mode>`, then Edit the JSON to replace every TODO with real, sourced content. Keep schema rules (N slides — default 8, slide1=cover, last=cta, alt_text length = N, score.total = sum, ≥1 source, reel beats filled, `video.caption_mode` = requested mode). Then **humanize** the caption/narration/on_slide_copy with the `humanizer` skill (+ `voice-profile.md`) → house voice per `pipeline/content/VOICE_AND_TONE_GUIDE.md`; never alter a sourced fact. Then **de-slop** with the `stop-slop` skill over caption/narration/on_slide_copy/subline/alt_text (cut filler, throat-clearing, AI jargon, vague declarations; active voice; score ≥35/50). Then **proofread** with the `professional-proofreader` skill: fix grammar/syntax, make every line a complete spoken sentence with substance (not telegraphic fragments), and apply dash/fragment hygiene (glue `first-ever`/`AI-assisting`; **NO em-dashes (`—`/`–`) on any surface and NO sentence fragments** — complete sentences only). Offensive depth is OK on offensive-theme posts when educational.
